...
const columns: ColumnDef<ProvincesType>[] = [
  {
...
const Provinces: React.FC = () => {
  ...
  const [deleteItem] = useDeleteProvinceMutation();
  const navigate = useNavigate();
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<ProvincesType | null>(null);

  const handleEdit = (province: ProvincesType) => {
    navigate(`/provincias/editar/${province._id}`);
  };

  const handleDelete = (item: ProvincesType) => {
      ...
      <div className="w-full p-8 bg-white rounded shadow-md">
        <div className="flex justify-between items-center mb-6 border-b pb-4 border-gray-400">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold text-gray-700">Provincias</h1>
          </div>
          <Link
            to="/provincias/nuevo"
